...
def drawROI(img, corners):
    ...
    for pt in corners:
        cv2.circle(cpy, tuple(pt.astype(int)), 25, c1, 2)
...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        for i in range(4):
            if dragSrc[i]:
                srcQuad[i] = (x, y)
                cpy = drawROI(resized, srcQuad)
                cv2.imshow('resized', cpy)
                ptOld = (x, y)
                break
...
dst = cv2.warpPerspective(resized, pers, (dw, dh), flags=cv2.INTER_CUBIC)
cv2.imshow('dst', dst)
cv2.waitKey()