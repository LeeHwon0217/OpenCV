...
for y in range(4):
    for x in range(4):
        ...
        cv2.threshold(img_, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU, dst_)  # 집어넣어라

# 적응형 이진화
# ADAPTIVE_THRESH_MEAN_C : 해당 픽셀 주변의 평균값을 기준으로 임계값 설정
# 9 : 이웃 블록의 크기 -> 현재 픽셀 주변에서 얼마만큼의 영역을 고려할 지 설정. 9*9. 클수록 더 넓은 영역을 평균 -> 조명에 덜 민감하지만 부드러움
# 5 : 임계값 보정 상수. 평균 또는 가중 평균에서 얼마나 빼줄지 결정. 값이 클수록 픽셀이 어두워야 흰색이 됨(임계값을 낯추거나 높이기 위한 튜닝용 파라미터)
