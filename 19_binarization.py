# 이진화
# 이미지의 픽셀 값을 0과 1(0과 255) 두 가지 값만 가지도록 만드는 영상처리 기법
# OCR, 윤곽 검출, 객체 분할, 문서 스캔 등의 작업에 유리
...
# 픽셀값이 임계값을 넘으면 최댓값으로 설정하고, 넘지 못하면 0으로 설정
...
# 오츠 임계값?
# 전체를 보고 연산을 하기 때문에 밝고 어둡고가 있으면 잘 찾지 못한다
