...
# GaussianBlur
# 픽셀 주변 값들을 가우시안 분포에 따라 가중평균에서 흐리게 만듦
# 커널 크기가 클 술록 넓은 범위를 흐리게 함
...
#     # k*k 크기 커널 생성 : 예) k = 5 면 1/25로 채워진 5*5 행렬
#     kernal = np.ones((k, k)) / k ** 2
#     # dst1 : 영상, -1 : 출력 이미지의 채널을 동일하게,
#     filtering = cv2.filter2D(dst1, -1, kernal)
#     plt.subplot(1, 3, i+1)
